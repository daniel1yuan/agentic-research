...
pub const REVISION_TOOLS: &[&str] = ANALYSIS_TOOLS_WITH_WEB;
...
mod tests {
    ...
    #[test]
    fn research_agents_have_web_tools_but_not_search() {
    ...
    #[test]
    fn revision_tools_include_everything() {
        assert!(REVISION_TOOLS.contains(&"WebSearch"));
        assert!(REVISION_TOOLS.contains(&"Glob"));
        assert!(REVISION_TOOLS.contains(&"Read"));
    }

    #[test]
    fn all_prompt_files_returns_nine() {
        assert_eq!(all_prompt_files().len(), 9);
    }
}
